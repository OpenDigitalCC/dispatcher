...
The CA private key never leaves the ctrl-exec host. Loss of the CA key means
...
: All `run`, `ping`, `capabilities`, and cert renewal traffic. Both ctrl-exec
...
: Each pairing request includes a 6-digit confirmation code derived from a
  ...
  time. The ctrl-exec displays the same code alongside the hostname and IP
...
: Each pairing request includes a 32-hex-character random nonce generated by
  the agent. The ctrl-exec stores it with the pending request and echoes it in
...
: `ctrl-exec-agent request-pairing` checks that `/etc/ctrl-exec-agent` is
  ...
  immediately rather than leaving a stale pending request in the ctrl-exec's
  pairing queue.
...
The ctrl-exec only renews certs for hosts in its registry. An agent that has
...
The ctrl-exec-side hook (configured via `auth_hook` in `ctrl-exec.conf`)
is called before every `run`, `ping`, `capabilities`, and API request. It
is the sole access control policy engine for the ctrl-exec — ctrl-exec
...
: When no hook is configured, behaviour depends on the caller. CLI invocations
  (`ctrl-exec run`, `ctrl-exec ping`) unconditionally pass - CLI access is
...
Token forwarding
: Tokens are included in the JSON payload sent from the ctrl-exec to the
...
Token logging
: Tokens are never logged by the ctrl-exec or the agent. They appear in the
...
During cert rotation, the ctrl-exec presents its new cert before agents
...
seconds; on a slow fleet it is bounded by the ctrl-exec's connection timeout.
...
The ctrl-exec cert is renewed automatically before expiry. The renewal
process and the serial tracking work together to rotate credentials without
disrupting the fleet:

Renewal trigger
: The ctrl-exec checks its own cert expiry at startup and every 4 hours
...
Broadcast
: Immediately after generating the new cert, the ctrl-exec calls
...
: Agents that were offline during the broadcast are marked `pending`. The
  ctrl-exec retries them on each subsequent check interval. After
...
: Shows the current and previous ctrl-exec serial, rotation timestamp,
...
: `ctrl-exec unpair <hostname>` removes the agent from the registry and
  ...
  to the ctrl-exec's own revocation check (if implemented) or decommission
  ...
  connecting to the ctrl-exec.

What revocation closes
: A compromised ctrl-exec cert that has been revoked cannot connect to any
  ...
  the ctrl-exec side - that requires the ctrl-exec-side equivalent (serial
...
- Back up `/etc/ctrl-exec/ca.key` to encrypted offline storage immediately
  after `setup-ca`
- Restrict access to the ctrl-exec host itself; the CA key should not be
...
- Audit access to the ctrl-exec host via system auth logs
...
: The agent tracks connection attempts per source IP in memory. A source
  ...
  side effect. A legitimate ctrl-exec that triggers the probe block due to a
...
where the API is reachable from outside the ctrl-exec host:
...
| Rogue ctrl-exec connecting to agent | Agent verifies ctrl-exec cert against CA |
...
| Lateral reconnaissance via capabilities | `/capabilities` restricted to stored ctrl-exec serial; re-pair to activate |
| Script execution by non-current ctrl-exec cert | `/run` and `/ping` restricted to stored ctrl-exec serial; hard deny on mismatch |
...
| Compromised ctrl-exec cert | Add serial to revoked-serials on all agents; reload |
| Token leaking via logs | Tokens never logged by ctrl-exec or agent |
...
monitoring and alerting recommendations, ctrl-exec host security requirements,
